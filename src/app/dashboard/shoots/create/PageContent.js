...
export default function CreateShootPage() {
  ...
  const handleStartChange = useCallback((val) => {
    setForm((prev) => {
      ...
      if (val && !prev.end_datetime) {
        const [datePart, timePart] = val.split("T");
        if (datePart && timePart) {
          let [h, m] = timePart.split(":").map(Number);
          h = (h + 1) % 24;
          next.end_datetime = `${datePart}T${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}`;
        }
      }
      return next;
    });
    prevStartRef.current = val;
  }, []);

  const handleEndChange = useCallback((val) => {
    setForm((prev) => ({ ...prev, end_datetime: val }));
  }, []);
